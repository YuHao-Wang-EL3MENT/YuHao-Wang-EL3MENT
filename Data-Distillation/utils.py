...
def get_dataset(dataset, data_path):
    if dataset == 'MNIST':
        ...
        mean = 0.5 #random
        std = 0.5 #random
        class_names = 'random' #random
        annotations = pd.read_csv('/content/drive/MyDrive/ECE1512/ProjectA/mhist_dataset/annotations.csv')
        annotations.set_index('Image Name', inplace=True)
        images_path = '/content/drive/MyDrive/ECE1512/ProjectA/mhist_dataset/images'
        ...
        for img in images_name:
            ...
            if annotations.loc[img]['Partition'] == 'train':
                img_train_origin = Image.open('/content/drive/MyDrive/ECE1512/ProjectA/mhist_dataset/images/' + img)
            ...
            if annotations.loc[img]['Partition'] == 'test':
                img_test_origin = Image.open('/content/drive/MyDrive/ECE1512/ProjectA/mhist_dataset/images/' + img)
        ...
        mhist_train_img = torch.from_numpy(mhist_train_img)
        mhist_train_label_le = torch.from_numpy(mhist_train_label_le)
        mhist_test_img = torch.from_numpy(mhist_test_img)
        mhist_test_label_le = torch.from_numpy(mhist_test_label_le)

        mhist_train_img = mhist_train_img.reshape(2175,3,224,224)

        mean = torch.tensor(mhist_train_img, dtype=torch.float32)
        std = torch.tensor(mhist_train_label_le)
        dst_test = TensorDataset(mhist_test_img, mhist_test_label_le)
        dst_train = 0
